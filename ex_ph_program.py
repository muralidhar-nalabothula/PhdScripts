...
ele_dips = get_dipoles(bs_bands,nvalance_bnds,dip_file=dipole_file,var='DIP_iR')
...
if Exph:
    ...
    for i in tqdm(range(qidx_in_kpts.shape[0]),desc="Exciton-phonon "):
        ...
        ## get rotated ex-wfc
        tik = time()
        ik_ibz, isym = kmap[qidx_in_kpts[i]] ## get the ibZ kpt and symmetry matrix for this q/k point
        ...
        tik = time()
        ex_ph_tmp = ex_ph_mat(wfc_tmp, BS_wfcs[0], eph_mat_iq, kpts[0], kpts[qidx_in_kpts[i]], kpts, kpt_tree)
...
if lumin:
    ...
    print(f'Minimum energy of the Direct exciton is : {min(BS_eigs[0])*27.2114:.4f} eV')
    ...
    ## save intensties
    np.savetxt('luminescence_intensities.dat',np.c_[ome_range, np.array(lumen_inten)] )
