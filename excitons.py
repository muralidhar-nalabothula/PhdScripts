...
def rotate_exc_wfc(wfc, symm_mat_red, kpoints, ktree, exe_qpt, dmats, time_rev):
    ...
    if time_rev:
        rot_wfc[:,idx_Rk,...] = pytorch.einsum('nkcv,kic,kjv->nkij',wfc_tmp.conj(), Dcc, Dvv)
    else :
        rot_wfc[:,idx_Rk,...] = pytorch.einsum('nkcv,kic,kjv->nkij',wfc_tmp, Dcc, Dvv)
...
def ex_ph_mat(wfc_k_q, wfc_k, elph_mat, qpt_exe, qpt_ph, kpts, ktree):
    ...
    #
    idx_k_minus_Q_minus_q   = find_kindx(kpts-qpt_ph[None,:]-qpt_exe[None,:],ktree) # k-Q-q
    ...
    wfc_k_electron = pytorch.from_numpy(wfc_k)[:,idx_k_minus_q,...].contiguous()
    wfc_k_hole     = pytorch.from_numpy(wfc_k)
    wfc_kq_conj    = pytorch.from_numpy(wfc_k_q).reshape(n_exe_states,-1).conj()
    #
    ex_ph_mat = pytorch.zeros((nmodes,n_exe_states,n_exe_states),dtype=pytor_Cmplx)
