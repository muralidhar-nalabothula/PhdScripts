...
def read_bse_eig(bse_folder='SAVE', iq=1, neig=1):
    ...
    bs_table = np.rint(diago_file['BS_TABLE'][...].data.T).astype(int) # (k,v,c)
    ...
    eig_wfcs_returned = np.zeros(eig_wfcs.shape,dtype=numpy_Cmplx) #(neig,nk,nc,nv)
...
def get_SAVE_Data(save_folder='SAVE'):
    ...
    symm_mats = ns_db1['SYMMETRY'][...].data.transpose(0,2,1).astype(numpy_float)
    ...
    ns_db1.close()
    return lat_vecs, nk_ibz.shape[0], symm_mats, int(np.rint(dims[9])) ## dim[9] is time_rev is on/off
...
def get_dipoles(bands_range, nval_bnds, dip_file='ndb.dipoles',var='DIP_iR'):
    ...
    if nvmax ==ncmmax and ncmin == nvmin:
        ...
        dipoles = ndb_dip[var][0,:,start_bnd_idx:val_bnd_idx,val_bnd_idx:end_bnd,...].data # kvc
    else :
...
def get_ph_data(bands_range, elph_file, dmat_file='ndb.Dmats'):
    ...
    Dmats = Dmat_data['Dmats'][:,:,0,start_bnd_idx:end_bnd,start_bnd_idx:end_bnd,:].data 
    Dmats = Dmats[...,0] + 1j*Dmats[...,1] # # nsym_ph, nkpts, Rk_band, k_band,
    Dmat_data.close()
    assert(nbnds == Dmats.shape[-1]) ## sanity check
    return ph_sym, time_rev, kpts, kmap, qpts, qmap, ph_freq*0.5, stard_conv, [start_bnd_idx,end_bnd], Dmats.astype(numpy_Cmplx)


def get_elph_data_iq(iq, elph_bands_range, stard_conv, ph_freq, ktree, kpts, qpts,  elph_file):
    start_bnd_idx = elph_bands_range[0]
    end_bnd = elph_bands_range[1]
    #print(start_bnd_idx, end_bnd)
    eph_mat = elph_file['elph_mat'][iq, :, :, 0, start_bnd_idx:end_bnd, start_bnd_idx:end_bnd,:].data
    eph_mat = eph_mat[...,0] + eph_mat[...,1]*1j #nk, nmodes, initial_band, final_band_PH_abs 0,1,2,3
    eph_mat = eph_mat.transpose(1,0,3,2) #nmodes,nk, final_band_PH_abs, initial_band
