              ...
              int_type start_bnd_idx, int_type end_bnd)
{
    ...
    {
        if (nc_get_vara_double(ncid, varid, start, count, (double*)raw_data) !=
            NC_NOERR)
        {
            free(raw_data);
            return -1;
    ...
    {
        if (nc_get_vara_float(ncid, varid, start, count, (float*)raw_data) !=
            NC_NOERR)
        {
            free(raw_data);
            return -1;
    ...
    if (*elph_out == NULL)
    {
        *elph_out = malloc(nmode * nk * nbnds * nbnds * sizeof(complex_t));
    }
    ...
    for (int_type m = 0; m < nmode; m++)
    {
        for (int_type k = 0; k < nk; k++)
        {
            for (int_type b1 = 0; b1 < nbnds; b1++)
            {
                ...
                {
                    // full_elph shape: (1, nk, nmode, 1, nband_full,
                    // nband_full)
                    int_type full_b1 = start_bnd_idx + b1;
                    int_type full_b2 = start_bnd_idx + b2;

                    int_type idx_full =
                        (((k * nmode + m) * 1 + 0) * nband_full + full_b1) *
                            nband_full +
                        full_b2;
                    complex_t val = full_elph[idx_full];

                    // Output python shape: (nmodes, nk, final_band,
                    // initial_band) So in C, we map output as: [m, k, b2, b1]
                    int_type idx_out = ((m * nk + k) * nbnds + b2) * nbnds + b1;
