...
omega_two_ph_freq = [
    1.0
]  ## list of frequecies in eV (not range liste omega_one_ph_range)
broading = 0.01  # in eV
npol = 3
...
nq_read = nq
if not two_ph and one_ph:
    nq_read = 1
#
elph_mat = np.zeros((nq_read, nmodes, len(kpts), nbnds, nbnds),
                    dtype=ele_dips.dtype)
...
if one_ph:
    ...
    omega_one_ph_freq = np.linspace(omega_one_ph_range[0],
                                    omega_one_ph_range[1],
                                    num=omega_one_ph_range[2])
    ram_ten_one_ph = []
