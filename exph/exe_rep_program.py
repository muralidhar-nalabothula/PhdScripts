...
frac_trans = np.zeros((symm_mats.shape[0],3)) ## yambo does not support frac trans
...
uni_eigs, degen_eigs = np.unique((BS_eigs/degen_thres).astype(int),return_counts=True)
...
if not read_symm_from_ns_db_file:
    ...
    frac_trans = elph_file['fractional_translation'][...].data ## this is in cart units
...
sym_red = np.einsum('ij,njk,kl->nil',lat_vecs.T,symm_mats,blat_vecs,optimize=True)
...
#isym = 2
    Sq_minus_q = np.einsum('ij,j->i',sym_red[isym],kpts_ibz[iQ-1])-kpts_ibz[iQ-1]
    #print(Sq_minus_q)
    ...
    tau_dot_k = np.exp(-1j*2*np.pi*np.dot(kpts_ibz[iQ-1],frac_trans[isym]))
    ...
    rep = np.einsum('n...,m...->nm',np.conj(wfc_tmp),BS_wfcs,optimize=True)*tau_dot_k
...
pg_label, classes, class_dict, char_tab, irreps = get_pg_info(symm_mats[little_group-1])
...
for i in range(len(trace_req)):
    rep_str_tmp = decompose_rep2irrep(trace_req[i], 
                                      char_tab, len(little_group), 
                                      class_orders, irreps)
